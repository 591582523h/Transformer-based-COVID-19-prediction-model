...
class train_Dataset(Dataset):
    def __init__(self, batchsize=32, input_sequence_length=14, output_sequence_length=7, file='data/china_covid.xlsx'):
        data = pd.read_excel(file)

        self.confirm = torch.tensor(list(data['ConfirmedChange']))
        self.x = torch.tensor()
        self.fx = torch.tensor()

        for i in range(len(confirm) - seq):
            self.x = torch.cat(self.x, [confirm[i:i + input_sequence_length]])
            self.fx = torch.cat(self.fx, [confirm[i + input_sequence_length: i + seq]])
